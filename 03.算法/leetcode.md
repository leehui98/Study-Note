...
动态规划，(i,j)依靠(i+1,j)和(i,j-1)。则 i ∈ [nums.size()-1，0]，j ∈ [i+1，nums.size()）

#### 3.灯泡开关

有n个灯泡是暗着的，第一次每个都按一次，第二次按2、4、..、n，第三次按3、6、9、...、n。第n次按n。求最后还剩下多少灯泡亮着。

最后还剩下(int)sqrt(n)个灯泡亮着。

灯泡亮着，就是被按了奇数次，第6个灯泡在（1，2，3，6）的时候被按了。第16个灯泡在第（1，2，4，8，16）次被按了，因为是4*4=16，所以相当于按了一次，所以最后只需要知道n之前有多少个平方和就可以了。对n开方是小数的话，相当于设置了一个上界，前面的数平方都在1到n的里面。所以最后的结果就是n的开方取整。 
